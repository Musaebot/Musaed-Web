...
Orientation for an AI agent picking this repo up — this is the reference manual; read it
before you touch anything. `README.md` is a short plain-language page for human visitors
(what the project is, how to run it) and is not a technical reference — don't look there for
history, reasoning, or implementation detail. That all lives here, including §11's material
folded in from README when it was rewritten to be simple (2026-08-25).
...
| `#stats` | مساعد بالأرقام | public counters (mock) — **`hidden`**, §1 |
...
- **Without JS the toggle button is hidden**, since it couldn't do anything. The footer is
  the fallback and must carry every nav destination (§6) — add a link to both or no-JS
  phones lose it.

### Motion: scroll-driven effects

Layer 1 (baseline) is the `IntersectionObserver`-driven `.reveal` staggering already covered
in §6. Layer 2 is progressive enhancement: native CSS scroll-driven timelines in one
...
specifically rather than its `.reveal` parent `.hero__visual` (§6's reveal/scroll-link
conflict rule).
...
`assets/Pics/Discord-Icon.png` (100×100, transparent) renders in the 5 places §5 counts, all
on `index.html`:
...
`#4e56c6` hover fill. Everything else stays green — see §5 for the scoping rule
(`.btn--primary` only) and the current `var(--accent)` usage count.
...
(currently `null`, §7) and adjust `shapeStats()` to match the payload's field names — nothing
...
taken from Fontsource. Icon sourcing is covered in §5 — same file, don't duplicate the
process description here.

---

## 12. Working style expected here
